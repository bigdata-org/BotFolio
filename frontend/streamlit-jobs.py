...
# def get_jobs():
#     """Get all job listings from the API"""
#     response = api_request("jobs", method="post", data={})
...
#     response = api_request("interview-prep", method="post", data={
#         "job_role": job_role,
#         "skills": skills,
#         "job_url": job_url
#     })
...
#     return api_request("cache-status", method="get")

# def refresh_cache():
#     """Manually refresh the cache"""
#     with st.spinner("Refreshing job data..."):
#         response = api_request("refresh-cache", method="post")
...
# def export_jobs_as_csv(filtered_df):
#     """Export filtered DataFrame as CSV"""
...
#             csv_data = export_jobs_as_csv(jobs_df)
#             if csv_data:
...
#             # Display jobs in a visually appealing way using cards
...
#                 job_key = f"{job_url}-{idx}"
                
#                 # Create a two-column layout for job card
#                 # Combine into one single column layout
#                 with st.container():
#                     job_card = f"""
#                     <div class="job-card">
...
#     else:
#         st.warning("No job data available. Try refreshing the data using the sidebar button.")
    ...
    if st.session_state.jobs_data is not None:
        ...
        if jobs_df.empty:
            ...
            for idx, job in jobs_df.iterrows():
                ...
                #         <div class="job-meta">
                #             <div><span class="job-role-tag">{job_role}</span>
                #             <span style="margin-left: 10px;">{job_seniority}</span>
                #             <span style="margin-left: 10px;">{job_employment}</span></div>
                #             <div class="job-date">Posted: {posted_date}</div>
                #         </div>
                ...
                #     <div class="job-buttons">
                #         <a href="{job_url}" target="_blank" class="view-button">View Job Listing</a>
                #         <a href="#" onclick="document.getElementById('prep-{job_key}').click(); return false;" class="prep-button">Prepare for Interview</a>
                #     </div>
                # </div>
                # """, unsafe_allow_html=True)
                ...
                #     st.session_state.interview_prep_job = job_key
                
                # # Show interview prep form if this job was selected
                # if st.session_state.interview_prep_job == job_key:
                #     with st.form(key=f"interview_form_{job_key}"):
                #         st.subheader(f"Interview Preparation for {job_title}")
                        
                #         # Pre-populate with skills if available
                #         skills_text = ", ".join(skills) if skills else ""
                #         interview_skills = st.text_area(
                #             "Skills to focus on (comma separated):",
                #             value=skills_text,
                #             help="Enter the skills you want to prepare for this interview"
                #         )
                        
                #         submit = st.form_submit_button("Get Preparation Materials")
                        
                #         if submit:
                #             skills_list = [s.strip() for s in interview_skills.split(",") if s.strip()]
                            
                #             if skills_list:
                #                 with st.spinner("Preparing interview materials..."):
                #                     response = request_interview_prep(job_role, skills_list, job_url)
                                    
                #                     if response and response.get("status") == "success":
                #                         st.success(response.get("message", "Success! Interview preparation request processed."))
                #                         # Clear the form after submission
                #                         st.session_state.interview_prep_job = None
                #                     else:
                #                         st.error("Failed to prepare interview materials. Please try again.")
                #             else:
